...
if ~isdeployed
  ...
  if strcmp(container, 'docker')
    cmd = ['docker run -i --rm -u 0:0 -v ' fullfile(filepath, 'seg') ':/input:ro -v ' fullfile(filepath, 'seg') ':/output 696021503801.dkr.ecr.us-east-1.amazonaws.com/prostate-segmentation:dev'];
  else
